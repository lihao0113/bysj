...
public class TaskService {
     ...
     * @return
     */
    public JSONObject myTaskAll(String searchPhrase, String current, String rowCount, SysUser sysUser) {
        JSONObject result = new JSONObject();
        try {
            List<Task> tasks = new ArrayList<Task>();
            int pageNumber = Integer.valueOf(current);
            int pageSize = Integer.valueOf(rowCount);
            int first = (pageNumber - 1) * pageSize;
            String assignName = sysUser.getUsername();
            List<Task> taskList = taskRepository.findByAssignName(assignName);
            if ("-1".equals(rowCount)) {
                tasks = taskRepository.findAllMyList(searchPhrase, assignName);
            } else {
                tasks = taskRepository.findMyTasks(searchPhrase, assignName, first, pageSize);
            }
            JSONArray array = new JSONArray();
            for (Task item : tasks) {
                array.add(item);
    ...
    public JSONObject add(SysUser currentUser) {
        JSONObject result = new JSONObject();
        try {
